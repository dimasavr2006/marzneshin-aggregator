...
def get_external_subscription(db: Session, sub_id: int) -> ExternalSubscription | None:
    return db.query(ExternalSubscription).filter(ExternalSubscription.id == sub_id).first()


def get_external_subscriptions(
    db: Session, admin_id: int | None = None, category: str | None = None
) -> list[ExternalSubscription]:
    query = db.query(ExternalSubscription)
    if admin_id is not None:
        query = query.filter(ExternalSubscription.admin_id == admin_id)
    if category is not None:
        query = query.filter(ExternalSubscription.category == category)
    return query.all()
...
def remove_external_subscription(db: Session, sub: ExternalSubscription):
    db.delete(sub)
    db.commit()
    return sub


def create_proxy_pool_server(db: Session, subscription_id: int, **kwargs) -> ProxyPoolServer:
    server = ProxyPoolServer(subscription_id=subscription_id, **kwargs)
    db.add(server)
    db.commit()
    db.refresh(server)
    return server
...
def get_proxy_pool_servers(
    ...
    query = db.query(ProxyPoolServer)
    if subscription_id is not None:
        query = query.filter(ProxyPoolServer.subscription_id == subscription_id)
    if is_available is not None:
        query = query.filter(ProxyPoolServer.is_available == is_available)
    return query.all()


def remove_proxy_pool_servers(db: Session, subscription_id: int):
    db.query(ProxyPoolServer).filter(
        ProxyPoolServer.subscription_id == subscription_id
