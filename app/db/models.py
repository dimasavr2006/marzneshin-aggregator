...
class ExternalSubscription(Base):
    ...
    server_selection_mode = Column(String(32), default="all", server_default="all")
    selected_server_ids = Column(JSON, default=list)
    admin_id = Column(Integer, ForeignKey("admins.id"))
    ...
    parsed_servers = Column(JSON, default=list)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class ProxyPoolServer(Base):
    __tablename__ = "proxy_pool_servers"
