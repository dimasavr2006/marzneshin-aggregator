...
from app.db.crud import get_hosts_for_user, get_external_subscriptions, get_proxy_pool_servers
from app.db.models import ExternalSubscription, ProxyPoolServer
...
def generate_subscription_html(
    db_user, subscription_settings: SubscriptionSettings
):
    """Generate HTML subscription page with categorized links."""
    links = generate_subscription(
        user=db_user,
        config_format="links",
        use_placeholder=not db_user.is_active
    ...
    for link in links:
        if link.startswith("vless://") or link.startswith("vmess://") or link.startswith("trojan://"):
            # Parse remark from URL fragment
            if "#" in link:
                remark = link.split("#")[-1]
                try:
                    remark = remark.replace("%20", " ").replace("+", " ")
...
def proxy_pool_server_to_v2data(
    ...
    data = V2Data(
        ...
        address=srv.address,
        port=srv.port,
        uuid=UUID(srv.uuid) if srv.uuid else None,
        password=srv.password,
        transport_type=srv.network or "tcp",
        tls=srv.tls,
        sni=srv.sni,
        host=srv.host,
        path=srv.path,
...
def get_proxy_pool_configs(
    ...
    with GetDB() as db:
        # Bridge servers
        bridge_subs = get_external_subscriptions(
            db, admin_id=user_admin_id, category="bridge"
        )
        bridge_servers = []
        ...
        external_subs = get_external_subscriptions(
            db, admin_id=user_admin_id, category="external"
        )
